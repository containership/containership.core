...
class Applications {
    ...
    get_discovery_port_key_variable(application_name) {
        const application_discovery_port = ['CS', 'DISCOVERY', 'PORT', application_name.toUpperCase()].join('_');
        return application_discovery_port;
    }

    get_application_address_variable(application_name) {
        const application_address = ['CS', 'ADDRESS', application_name.toUpperCase()].join('_');
        return application_address;
    }

    cleanup_autogenerated_variable(application_name, callback) {
        const application_discovery_port = this.get_discovery_port_key_variable(application_name);
        const application_address = this.get_application_address_variable(application_name);

        return async.each([application_discovery_port, application_address], (variable_name, callback) => {
            return this.core.cluster.myriad.persistence.delete([constants.myriad.AUTOGENERATED_VARIABLES_PREFIX, variable_name].join(constants.myriad.DELIMITER), callback);
        }, (err) => {
            if(err) {
                this.core.loggers['containership.core'].log('error', `Error deleting autogenerated varriables for ${application_name}: ${JSON.stringify(err)}`);
            }
    ...
    add(config, add_callback) {
        ...
        this.core.cluster.myriad.persistence.keys(constants.myriad.APPLICATIONS, (err, applications) => {
            ...
            if(!_.includes(applications, [constants.myriad.APPLICATION_PREFIX, application_name].join(constants.myriad.DELIMITER))) {
                _.defaults(config, {
                    id: null,
                    tags: {},
                    env_vars: {},
                ...
                self.core.scheduler.leader.application.get_loadbalancer_port(config.port, (err, discovery_port) => {
                    ...
                    self.core.cluster.myriad.persistence.set([constants.myriad.APPLICATION_PREFIX, application_name].join(constants.myriad.DELIMITER), JSON.stringify(config), (err) => {
                        if(err) {
                            return add_callback(err);
                        }
                        ...
                        const application_discovery_port = self.get_discovery_port_key_variable(application_name);
                        auto_variables[application_discovery_port] = config.discovery_port;

                        const application_address = self.get_application_address_variable(application_name);
                        auto_variables[application_address] = [application_name, self.core.cluster_id, 'containership'].join('.');

                        return async.eachOf(auto_variables, (value, variable_name, callback) => {
                            self.core.cluster.myriad.persistence.set([constants.myriad.AUTOGENERATED_VARIABLES_PREFIX, variable_name].join(constants.myriad.DELIMITER), value, callback);
                        }, (err) => {
                            if(err) {
                                return self.remove(application_name, () => {
                                    return self.cleanup_autogenerated_variable(application_name, () => {
                                        return add_callback(err);
                                    });
                                });
                            }
                            ...
                            async.each(containers, (container, callback) => {
                                ...
                                self.deploy_container(application_name, container, () => {
                                ...
                                return self.core.scheduler.leader.application.start_health_check(application_name, (err) => {
    ...
    remove(application_name, remove_callback) {
        ...
        async.series([
            ...
            (callback) => {
                return self.cleanup_autogenerated_variable(application_name, callback);
            },
            (callback) => {
